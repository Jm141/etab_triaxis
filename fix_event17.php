...
echo "=== Fixing Event 17: rew Advancement ===\n";

// Clear all qualifications for this event
$db->query("UPDATE contestants SET qualified_for_level_id = NULL WHERE event_id = 17");
echo "All qualifications cleared.\n";

$scoringEngine = new ScoringEngine();

// Get rounds for Level 1 and Level 2
$level1Rounds = $db->fetchAll("SELECT id FROM rounds WHERE level_id = 48");
$level2Rounds = $db->fetchAll("SELECT id FROM rounds WHERE level_id = 49");
...
echo "\n=== Summary ===\n";
$level2Qualified = $db->fetchAll("SELECT * FROM contestants WHERE event_id = 17 AND qualified_for_level_id = 49");
$level3Qualified = $db->fetchAll("SELECT * FROM contestants WHERE event_id = 17 AND qualified_for_level_id = 50");
