...
class JudgeScoringController extends Controller {
    ...
    public function roundTable($roundId) {
        ...
        // show the top advance_count contestants from previous level
        if ($previousLevel && $previousLevel['advance_count'] !== null) {
            // Get contestants qualified for this level first (may be set by previous ranking calculations)
            $qualifiedContestants = $this->db->fetchAll(
                "SELECT c.*
                 FROM contestants c
                 WHERE c.event_id = ? 
                 AND c.status = 'Active'
            ...
            // If qualified contestants exist, use them
            if (!empty($qualifiedContestants)) {
                $contestants = $qualifiedContestants;
            } else {
                // Otherwise, get top advance_count contestants from previous level
                // Use all scores (both submitted and draft) for advancement calculation
                $contestants = $this->db->fetchAll(
                    "SELECT c.*, AVG(s.total_score) as avg_score
                     FROM contestants c
                     ...
                     AND r.level_id = ?
                     GROUP BY c.id
                     ORDER BY avg_score DESC, CAST(c.contestant_number AS UNSIGNED)
                     LIMIT ?",
                    [$round['event_id'], $previousLevel['id'], $previousLevel['advance_count']]
                );
            }
        } else {
        ...
        // Only include rounds from ongoing events and active assignments
        $assignedRounds = $this->db->fetchAll(
            "SELECT r.id, r.name, el.name as level_name, e.name as event_name
             FROM judge_assignments ja
             JOIN judges j ON ja.judge_id = j.id
             JOIN rounds r ON ja.round_id = r.id
             JOIN event_levels el ON r.level_id = el.id
